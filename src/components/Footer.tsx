...
export function Footer() {
  return <footer className="border-t border-[#e9eff5] bg-[#f8fbfd]">
    <div className="mx-auto flex max-w-[1200px] flex-col items-center justify-between gap-4 px-4 py-6 text-[11px] text-muted sm:px-6 lg:flex-row lg:px-8">
      ...
      <nav aria-label="لینک‌های پایین صفحه" className="flex flex-wrap justify-center gap-x-3 gap-y-2">{navigation.map(item => <a key={item.href} href={item.href} className="rounded px-1 py-1 hover:text-teal focus-visible:outline-2 focus-visible:outline-teal">{item.label}</a>)}</nav>
